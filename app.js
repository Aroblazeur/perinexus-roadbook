...
function renderRoadbookCurrentSummary(container, summary) {
    ...
    appendSummaryMap(section, summary.mapEmbedUrl, {
        ...
        className: "roadbook-current-summary__map map-embed"
    });
...
function appendSummaryMap(container, mapEmbedUrl, options = {}) {
    const embedUrl = window.roadbookMapViewer?.resolveExternalMapEmbedUrl?.(mapEmbedUrl);
    if (!embedUrl) return;

    const {
        className = "official-itinerary__map map-embed",
...
function renderStageMapEmbed(mapEmbedUrl, gpxUrl) {
    ...
    if (!viewer || typeof viewer.renderEmbed !== "function" || typeof viewer.renderGpx !== "function") {
        const section = document.getElementById("map-embed-section");
        if (section) section.hidden = true;
        renderMapGpxActions(resolvedGpx);
        console.error("[Map] Viewer indisponible.");
        return false;
    }

    const externalMapVisible = viewer.renderEmbed(mapEmbedUrl);
    if (!externalMapVisible && resolvedGpx) {
        viewer.renderGpx(resolvedGpx);
    }
    if (!externalMapVisible && !resolvedGpx) {
        const section = document.getElementById("map-embed-section");
        if (section) section.hidden = true;
    ...
    renderMapGpxActions(resolvedGpx);
    return externalMapVisible || Boolean(resolvedGpx);
}
